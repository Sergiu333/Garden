...
const Filter = ({produses, max}) => {
    ...
    const filteredProducts = produses.data.filter((product:any) => {
        ...
        // console.log(produses.map((link)=>link.attributes.poster.data.attributes.url), '222222222222222222222222222222222222222222222222222');

    const [imageBase64, setImageBase64] = useState<string | null>(null);

    useEffect(() => {
        convertImageToBase64(`${filteredProducts.map((link:any)=>link.attributes.poster.data.attributes.url)[0]}`)
            ...
            <div className="px-[20px] sm:px-[100px] md:px-[150px] flex flex-col relative mt-[50px] 2xl:mt-[70px]">
                    ...
                    <div className="py-[11px] px-[24px] bg-[#FFFBFB] rounded-[8px]">
                        <div className="flex flex-row gap-[14px] text-[#1B1B1B] items-center">
                            ...
                            <div className="relative w-[24px] h-[24px]">
                                <Image src="/filter.svg" fill={true} style={{objectFit: 'contain'}} alt="this image is from filter" priority={true} quality={100}/>
                            </div>
                        </div>
                    </div>
                </div>
                <div
                    ...
                    {filteredProducts.slice(0, max).map((product:any, index:any) => (
                        <Link href={'/project/' + product.id} className="flex flex-row gap-[32px] justify-center" key={`${product.id}`}>
                            <div className="flex flex-col bg-[#2C2B2B] w-fit rounded-[8px] overflow-hidden">
                                <div className="relative w-[380px] lg:w-[577px] h-[300px] lg:h-[450px]">
                                    <Image
                                        ...
                                        fill={true} style={{objectFit: 'cover'}} alt="this image is from filter" priority={true} quality={100}/>
                                </div>
                    ...
                    <Link href="/project">
                        <div className="flex justify-center items-center flex-col gap-1 py-[35px]">
                            <div className="relative w-[48px] h-[48px]">
                                <Image src="/circle.svg" alt="view more" fill objectFit="contain" placeholder="blur" blurDataURL="/circle.svg"/>
                            </div>
